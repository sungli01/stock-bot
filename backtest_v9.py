#!/usr/bin/env python3
"""
Backtest v9: 라이브 봇 최종 설정 통합 (2026-02-18)
v6 기반 + 추가 필터:
  1. max_price $10 (페니스탁만)
  2. 100%+ 이미 오른 종목 매수 제외 (고점 추격 방지)
  3. 같은 날 한번 거래/시도한 종목 재매수 금지
  4. 급등 초기(5~30%) 우선 정렬

매수: 1분봉 거래량 100%+ 급등 + 동시/이후 10%+ 가격 급등
매도: v6 동일 (30%+BB돌파+트레일링, 절대손절-50%, 장마감청산)
"""
...
MAX_POSITIONS = 2
MIN_PRICE = 0.7
MAX_PRICE = 10.0          # v9: 페니스탁 상한 $10
MAX_GAIN_EXCLUDE = 1.0    # v9: 100%+ 이미 오른 종목 제외
STOP_LOSS_PCT = -0.50
BB_PERIOD = 20
BB_STD = 2
TOP_N_CANDIDATES = 7
...
def get_day_gainers(date_str):
    ...
    for r in results:
        ...
        # v9: 100%+ 이미 오른 종목 제외
        if change_pct >= MAX_GAIN_EXCLUDE * 100:
            continue
    ...
    # v9: 급등 초기(5~30%) 우선, 이미 많이 오른 종목보다 낮은 우선순위
    def sort_key(x):
        cp = x['change_pct']
        # 5~30% 범위 우선 (score 2), 30~60% (score 1), 60%+ (score 0)
        if cp <= 30:
            tier = 2
        elif cp <= 60:
            tier = 1
            ...
            tier = 0
        return (tier, x['change_pct'] * math.log10(max(x['volume'],1)))
...
def run_backtest():
    print("=" * 60)
    print("Backtest v9: 라이브 봇 최종 설정 (v6 + max$10 + 고점추격방지 + 재매수금지)")
    print("=" * 60)
    ...
    for day_idx, date_str in enumerate(trading_days):
        ...
        day_trades = []
        positions_used = 0
        day_traded_tickers = set()  # v9: 같은 날 재매수 금지

        for cand in candidates:
            if positions_used >= MAX_POSITIONS:
                break
            ticker = cand['ticker']
            # v9: 같은 날 이미 거래/시도한 종목 스킵
            if ticker in day_traded_tickers:
                continue
            day_traded_tickers.add(ticker)
    ...
    if prev_versions:
        ...
        print("버전 비교표")
        print(f"{'='*60}")
        print(f"{'버전':<6} {'최종자본':>14} {'수익률':>10} {'MDD':>8} {'거래수':>6} {'승률':>7} {'평균승':>8} {'평균패':>8}")
        print("-" * 70)
        for v, s in sorted(prev_versions.items()):
            print(f"{v:<6} ₩{s['final_capital']:>12,} {s['total_return_pct']:>+9.1f}% {s.get('max_drawdown_pct',0):>7.1f}% {s['total_trades']:>5} {s['win_rate']:>6.1f}% {s.get('avg_win_pct',0):>+7.1f}% {s.get('avg_loss_pct',0):>+7.1f}%")
        print(f"{'v9':<6} ₩{capital:>12,.0f} {final_return:>+9.1f}% {max_drawdown:>7.1f}% {total_trades:>5} {float(wr):>6.1f}% {avg_win:>+7.1f}% {avg_loss:>+7.1f}%")
        print("-" * 70)
    ...
    # Save JSON
    json_path = '/home/ubuntu/.openclaw/workspace/stock-bot/backtest_v9_result.json'
